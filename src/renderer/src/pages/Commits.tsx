...
export default function Commits() {
  ...
  const countdown = useAutoRefresh(() => { setPage(1); load(1) })
  useKeyboardShortcuts({ 'r': () => load(1), 'R': () => load(1) })

  const copySHA = async (sha: string) => {
    await navigator.clipboard.writeText(sha)
    addToast(`SHA скопирован: ${sha.slice(0, 7)}`, 'success')
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 14 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexShrink: 0 }}>
        <div>
          <span style={{ fontFamily: 'var(--font-head)', fontSize: 11, color: 'var(--text-3)', letterSpacing: '0.1em' }}>КОММИТЫ </span>
          {defaultBranch && <span style={{ fontFamily: 'var(--font-mono)', fontSize: 12, color: 'var(--text-2)' }}>в ветке {defaultBranch}</span>}
        </div>
        <NeonButton size="sm" variant="ghost" onClick={() => load(1)} icon="↻" tooltip="Обновить список коммитов с GitHub">
      ...
      <div className="panel" style={{ flex: 1, overflow: 'auto' }}>
        ...
        {commits.map((c, i) => {
          ...
          return (
            <motion.div key={c.sha}
              initial={{ opacity: 0, x: -8 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.015 }}
              style={{ padding: compactMode ? '8px 14px' : '12px 16px', borderBottom: '1px solid var(--border)', display: 'flex', gap: 12, alignItems: 'flex-start' }}
            >
              {c.author
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ fontFamily: 'var(--font-body)', fontSize: compactMode ? 13 : 14, color: 'var(--text)', fontWeight: 500, marginBottom: 2, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{firstLine}</div>
                {body && !compactMode && (
                  <div style={{ fontFamily: 'var(--font-body)', fontSize: 12, color: 'var(--text-3)', marginBottom: 4, overflow: 'hidden', maxHeight: 36 }}>{body}</div>
                )}
                {!compactMode && (
              ...
              <div style={{ display: 'flex', gap: 4, flexShrink: 0 }}>
                {/* Copy SHA */}
                <button onClick={() => copySHA(c.sha)}
                  title="Скопировать полный SHA"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 5, color: 'var(--text-3)', fontFamily: 'var(--font-mono)', fontSize: 11, padding: '3px 6px', cursor: 'pointer', transition: 'all 0.1s' }}
                ...
                >⎘</button>
                {/* Open on GitHub */}
                <button onClick={() => window.open(c.html_url, '_blank')}
                  title="Открыть на GitHub"
                  style={{ background: 'rgba(255,255,255,0.04)', border: '1px solid var(--border)', borderRadius: 5, color: 'var(--blue)', fontFamily: 'var(--font-mono)', fontSize: 11, padding: '3px 8px', cursor: 'pointer', transition: 'background 0.1s' }}
        ...
        {!loading && hasMore && (
          <div style={{ padding: 16, textAlign: 'center' }}>
            <NeonButton size="sm" variant="ghost" onClick={loadMore}>Загрузить ещё</NeonButton>
          </div>
        )}
      </div>
    </div>
